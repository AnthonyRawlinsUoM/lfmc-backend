...
import logging
import json
from serve.lfmc.library.geoserver.layer import Layer
from serve.lfmc.library.geoserver.resource import FeatureType, Coverage
from serve.lfmc.library.geoserver.store import coveragestore_from_index, datastore_from_index, \
    wmsstore_from_index, UnsavedDataStore, \
    UnsavedCoverageStore, UnsavedWmsStore
from serve.lfmc.library.geoserver.style import Style
from serve.lfmc.library.geoserver.support import prepare_upload_bundle, url, _decode_list, _decode_dict, JDBCVirtualTable
from serve.lfmc.library.geoserver.layergroup import LayerGroup, UnsavedLayerGroup
from serve.lfmc.library.geoserver.workspace import workspace_from_index, Workspace
import os
import httplib2
...
class Catalog(object):
    ...
    def save(self, obj, content_type="application/xml"):
        ...
        if 400 <= int(headers['status']) < 600:
            raise FailedRequestError("Error code (%s) from serve.lfmc.library.geoserver: %s" %
                                     (headers['status'], body))
        return response
    ...
    def get_store(self, name, workspace=None):
        ...
        if stores.__len__() == 0:
            ...
            multiple_stores = []
            for s in stores:
                multiple_stores.append(
                    "{workspace_name}:{store_name}".format(workspace_name=s.workspace.name, store_name=s.name))

            raise AmbiguousRequestError(
                "Multiple stores found named {name} - {stores}".format(name=name, stores=", ".join(multiple_stores)))
        else:
            return stores[0]
    ...
    def set_default_workspace(self, name):
        ...
        if workspace is not None:
            ...
            assert 200 <= headers.status < 300, "Error setting default workspace: " + str(
