...
import logging
import json
from lfmc.library.geoserver.layer import Layer
from lfmc.library.geoserver.resource import FeatureType, Coverage
from lfmc.library.geoserver.store import coveragestore_from_index, datastore_from_index, \
    wmsstore_from_index, UnsavedDataStore, \
    UnsavedCoverageStore, UnsavedWmsStore
from lfmc.library.geoserver.style import Style
from lfmc.library.geoserver.support import prepare_upload_bundle, url, _decode_list, _decode_dict, JDBCVirtualTable
from lfmc.library.geoserver.layergroup import LayerGroup, UnsavedLayerGroup
from lfmc.library.geoserver.workspace import workspace_from_index, Workspace
import os
import httplib2
...
class Catalog(object):
    ...
    def save(self, obj, content_type="application/xml"):
        ...
        if 400 <= int(headers['status']) < 600:
            raise FailedRequestError("Error code (%s) from lfmc.library.geoserver: %s" %
                                     (headers['status'], body))
        return response
